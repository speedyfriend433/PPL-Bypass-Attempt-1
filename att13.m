...
kern_return_t kb_init_kernel(DebugLogger *logger) {
    ...
    uint64_t base = get_kernel_base(logger);
    if (base) {
        [logger logMessage:[NSString stringWithFormat:@"[DEBUG] Using kernel_base: 0x%llx", base]];
    }
    return KERN_SUCCESS;
}
...
+ (void)runSPTMBypassWithLogger:(DebugLogger *)logger {
    ...
    uint64_t kernel_base = get_kernel_base(logger);
    const uint64_t KERNEL_PMAP_ADDR = kernel_base != 0 ? kernel_base + 0x1000000 : 0x0;
    const uint64_t TASK_STRUCT_ADDR = kernel_base != 0 ? kernel_base + 0x2000000 : 0x0;
