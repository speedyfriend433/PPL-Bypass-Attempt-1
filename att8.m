...
+ (void)runTest {
    [self log:@"[INFO] Starting bypass with kernel..."];
    ...
    if (cvResult == kCVReturnSuccess && pixelBuffer) {
        ...
        if (cvSurface) {
            ...
            io_connect_t altConnection;
            kr = IOServiceOpen(surface, mach_task_self(), 0, &altConnection);
            
            if (kr == KERN_SUCCESS) {
                uint64_t inputValues[2] = {
                    IOSurfaceGetID(cvSurface),
                    0
                };
                size_t outputSize = sizeof(uint64_t) * 2;
                uint64_t outputValues[2] = {0};
                
                for (uint32_t selector = 0; selector < 5; selector++) {
                    kr = IOConnectCallMethod(altConnection,
                                           selector,
                                           inputValues, 2,
                                           NULL, 0,
                    ...
                    if (kr == KERN_SUCCESS) {
                        [self log:@"[SUCCESS] Found working selector: %d", selector];
                        break;
                    }
                }
                
                IOServiceClose(altConnection);
            }
    ...
    kr = IOConnectCallScalarMethod(connection,
                                  6,  // Different method index for client creation
                                  inputScalar,
                                  3,
                                  clientOutput,
                                  &clientOutputCount);
